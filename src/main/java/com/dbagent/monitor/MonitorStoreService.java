 ...
 * instance_metric_history와 같은 전용 저장소(metrics.db, metricsJdbcTemplate)에 둔다 - 60초/10분 주기 쓰기가
 ...
 *
 * <p>파일 크기: 보관 기간이 지나 지운 공간은 SQLite가 다음 쓰기에 재사용하므로 일정 기간 뒤로는 파일이 더 커지지
 * 않는다. VACUUM은 DB 전체에 쓰기 락을 오래 잡아(샘플러 쓰기가 막힘) 자동으로 돌리지 않는다 - 비밀번호 재암호화
...
public class MonitorStoreService {
    ...
    private static final String AUDIT_ID_DDL = "audit_id INTEGER PRIMARY KEY AUTOINCREMENT";

    private static String upsert(String table, String columns, String keyColumns, int paramCount) {
        StringBuilder q = new StringBuilder("INSERT OR REPLACE INTO ").append(table).append(" (").append(columns).append(") VALUES (");
    ...
    private int deleteChunk(String table, String tsColumn, long cutoff) {
        return jdbc.update("DELETE FROM " + table + " WHERE rowid IN (SELECT rowid FROM " + table +
                " WHERE " + tsColumn + " < ? LIMIT " + DELETE_CHUNK_ROWS + ")", cutoff);
    }
