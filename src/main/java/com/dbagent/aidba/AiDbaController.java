...
public class AiDbaController {
    ...
    public ResponseEntity<Map<String, Object>> health() {
        try {
            Map<String, Object> result = ollamaChatService.health();
     ...
     * 사내 사례를 찾는 게 아니라 눈앞의 실측치를 읽는 작업이라 RAG 검색은 타지 않는다(OllamaChatService.
     * askWithPrompt - /api/chat, /api/query 아님).
     */
    ...
    public ResponseEntity<Map<String, Object>> analyzeCurrentSql(@RequestBody CurrentSqlAnalyzeRequest req) {
        ...
        if (query == null || query.isBlank()) {
            return ResponseEntity.ok(Map.of("success", false, "message", "분석할 쿼리가 없습니다."));
        }
        if (plan == null || plan.isBlank()) {
            return ResponseEntity.ok(Map.of("success", false, "message", "먼저 1차 성능점검을 실행해주세요."));
        }
        try {
            ...
            return ResponseEntity.ok(Map.of("success", false, "message", ollamaChatService.friendlyErrorMessage("AI 분석", e)));
        }
    }
    ...
    private String buildCurrentSqlPrompt(String query, Map<String, String> binds, String plan) {
        ...
        if (binds != null && !binds.isEmpty()) {
            sb.append("[바인드 변수]\n");
            binds.forEach((k, v) -> sb.append(":").append(k).append(" = ").append(v).append("\n"));
            sb.append("\n");
        }
