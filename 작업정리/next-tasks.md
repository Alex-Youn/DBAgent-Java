...
> 작성 2026-09-07 · 최종 갱신 2026-09-14
> 항목별 상태를 제목에 표시한다(**완료** / **불필요** / 표시 없으면 미착수).
> 대상: `DBAgent-Java` / `DBAgent-Java-AIX` (2026-08-26 확정 원칙 — 두 프로젝트는 항상 같이 진행)

---

## -1. GPU 서버 `sql_tuning_cases` 인덱스 중복/오염 (2026-09-14 발견) — **사용자 확인 대기**

**증상:** "AI SQL 성능분석"(SQL Tune Advisor) 참조자료 중 일부만 한글이 깨져 나옴("일부만
깨진다"). GPU 서버 `LANG`은 UTF-8로 정상.

**원인 (확인됨):** `rocky9-server` 컨테이너의 `OpenSearchIndexerService.indexData()`
(`TestRunner`=`sql_tuning_cases`, `ErrorDictIndexerRunner`=`error_dictionary` 공용 로직)가
문서를 색인할 때 `POST /{index}/_doc`에 `_id`를 지정하지 않는다. `createIndexIfNotExists()`도
인덱스가 없을 때만 생성할 뿐 기존 내용을 비우지 않는다. 즉 `run_indexer.sh`를 실행할 때마다
기존 문서를 덮어쓰지 않고 전부 새 문서로 누적된다.

**증거:**
- 현재 소스 `tuning_data.jsonl`은 1301줄이며 자체 제작 스캐너(`scan_mojibake.py`)로 검사 시
  U+FFFD/mojibake 의심 0건 — 완전히 깨끗함.
- 그런데 실제 `sql_tuning_cases` 인덱스에는 3094건이 들어있고, 그중 2066건(67%)에서 U+FFFD가
  발견됨. 3094 vs 1301의 불일치와 다수의 깨진 문서는, 과거(파일이 지금처럼 정리되기 전) 최소
  한 번 이상 실행된 색인 결과가 `_id` 미지정 때문에 지워지지 않고 최신 클린 실행분과 함께 인덱스에
  섞여 남아있는 것으로 설명됨. 같은 문서 안에서 일부 문단만 깨지는 현상도 "레코드 자체가 오염된
  옛 실행분"과 "깨끗한 최신 실행분"이 검색 결과에 무작위로 섞여 나오는 것과 일치.
- 컨테이너 전체에 원본 `.hwp` 파일은 존재하지 않음(`find / -iname '*.hwp'` 0건) — HWP 재추출
  경로는 현재 불필요해 보임(재추출 없이도 기존 클린 jsonl로 재색인하면 해결될 가능성이 높음).

**제안 조치 (실행 전 사용자 확인 필요 — GPU 서버 공유 인덱스를 지우는 작업):**
1. `DELETE /sql_tuning_cases` 로 인덱스 삭제
2. `run_indexer.sh`(profile=indexer) 재실행 → 현재 클린한 `tuning_data.jsonl`(1301건)만 재색인
3. 색인 후 문서 수가 1301인지, mojibake 스캔 결과 0건인지 재확인
4. `error_dictionary` 인덱스는 로컬 `oracle_errors.db`(9372건, 깨진 건 2건)와 문서 수/내용이
   정확히 일치함을 이미 확인했으므로 별도 재색인 불필요 — 다만 원하면 문서 수만 추가 확인 가능.

> 근본 원인(POST `_doc`에 `_id` 미지정으로 재실행 시 중복 누적)은 코드 결함이지만, `sqlrestapi`는
> 이 세션의 스코프 밖(별도 Java 프로젝트, GPU 서버 내부)이라 코드 수정은 별도 판단 필요.
