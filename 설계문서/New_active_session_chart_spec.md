...
- 다크 모드: 라이트 모드 대비 대비비가 전반적으로 개선되므로 기본값을 다크로 두는 것도 고려 가능
...
- RAC 환경에서는 `GV$ACTIVE_SESSION_HISTORY` / `GV$ACTIVE_SESSION_HISTORY` + `INST_ID`로 인스턴스별
  데이터를 취합하고, 화면에는 합산 또는 인스턴스 필터를 제공합니다.
...
-- RAC: 인스턴스별 값을 합산
SELECT SUM(value) AS cpu_count FROM gv$parameter WHERE name = 'cpu_count';
```
...
- [ ] §2 색상/순서 고정 유지 — 임의 재정렬 금지
- [ ] 범례 상시 표시, 텍스처 토글, 표 보기 토글 — 접근성 3종 세트 구현
