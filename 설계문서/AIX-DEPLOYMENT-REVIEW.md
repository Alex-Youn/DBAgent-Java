...
- [ ] **Spring Boot 3.3.5 → 2.7.x 다운그레이드** (Java 8 요구사항, 5번 "Java 8 다운그레이드 시 코드 변경 범위" 표 참고: jakarta→javax, record→class, `List.of`/`Map.of`/`Set.of` 교체, `var` 제거, `ojdbc11`→`ojdbc8`)
- [ ] SQLite → H2 마이그레이션 (users.db, oracle_errors.db) — **H2는 1.4.200으로 고정** (2.x는 Java 11+ 필요)
- [ ] Ollama 챗봇 기능 제거 (`OllamaChatService`, `/api/aidba/chat`, 프론트 UI) — `java.net.http.HttpClient`(Java 11+ 전용) 의존성도 같이 제거됨
