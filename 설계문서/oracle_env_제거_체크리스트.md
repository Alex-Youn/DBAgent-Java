...
**남은 것(폐쇄망에서만 가능)**
- [ ] 폐쇄망 IBM J9의 `Cipher.getMaxAllowedKeyLength("AES")` 실측(개발기는 256이라 32바이트 키 생성됨. 128이면 코드가 자동 폴백하므로 기동은 되지만 실제 값 확인 필요)
- [ ] 키 파일을 앱 디렉터리 밖으로 이동 + `dbagent.security.key-file` 지정 + 별도 장소에 사본 1부 보관(운영 전환 시)
