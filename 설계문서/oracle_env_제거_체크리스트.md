...
> **2026-09-21 야간 작업으로 구현까지 완료.** 상세 설계와 구현 결과는
> `설계문서/비밀번호_암호화_설계_2026-09-21.md`, 그날 작업 내역은 `작업정리/작업정리_2026-09-21-야간.md` 참고.
> 커밋: main `2a379d3`, AIX `a286740`.

- [x] **현황 조사**: 대상은 `db_instances.password` 컬럼과 `accounts` JSON 안의 각 password 두 군데로 확정. `oracle.env`/`databases.json`은 4~5단계에서 이미 소멸, `users.db`의 로그인 계정 비밀번호는 **이미 BCrypt 해시라 범위 밖**(확인 완료)
- [x] **암호화 방식 결정**: 표준 `javax.crypto`의 AES/GCM/NoPadding, 12B 랜덤 IV, 128비트 태그, AAD=헤더. 외부 라이브러리 0개. 키 길이는 가변(256 우선, JVM이 거부하면 128 자동 폴백)
- [x] **키 관리 방안 결정**: 환경별 독립 키, 키 파일 최초 기동 시 자동 생성, 환경변수 `DBAGENT_MASTER_KEY`는 선택적 오버라이드, POSIX면 600 자동 설정, `.gitignore`로 커밋 차단
- [x] **키 로테이션 정책 검토**: 키 파일에 `key.<id>` 다중 보유 + `active` 전환 → 재기동 시 기동 스캔이 자동 재암호화. 정기 주기는 연 1회 권고(유출 의심 시 즉시 + 대상 DB 비밀번호도 함께 변경)
- [x] **구현**: `com.dbagent.security.MasterKeyProvider` / `CredentialCipher` 신규. `DatabaseConfigService.resolvePassword/encodePassword`를 시그니처 유지한 채 위임으로 교체(호출부 무변경)
- [x] **기존 저장값 마이그레이션**: 1회성 스크립트가 아니라 **기동 시 멱등 스캔**(`reencryptAllPasswords()`) — 롤백/키 로테이션/부분 실패에서 자기치유되도록 플래그 방식을 일부러 피함
- [x] **회귀 확인**: 복호화된 값으로 실제 도커 MySQL 접속 성공(main/AIX 양쪽), 관리 화면 신규등록/빈비번유지/삭제, 재기동 멱등성, 깨진 B64 섞였을 때 기동 지속까지 확인
  - 단 Oracle은 로컬 인스턴스가 host 빈값이라 접속 검증 불가(5단계 alias 제거 영향) — 폐쇄망에서 확인 필요
- [x] main/AIX 동일 반영(dual-project-sync), 빌드/배포(3곳), 커밋+push

**설계에 없었는데 구현 중 발견해 추가로 처리한 것**
- [x] **재암호화 후에도 파일에 예전 B64 값이 남는 문제**(B64는 키 없이 디코딩되므로 파일 유출 시 암호화가 무의미해짐). SQLite는 `wal_checkpoint(TRUNCATE)`+`VACUUM`으로 기동 중 즉시 정리, H2는 온라인 수단이 없어 `DEFRAG_ALWAYS=TRUE`로 정상 종료 시 정리
- [x] **마이그레이션 중 손상된 값이 기동을 죽이는 회귀 수정**: `databases.json`에 깨진 `B64(...)`가 하나라도 있으면 `@PostConstruct`가 터져 앱이 아예 못 뜨던 것을, 해당 인스턴스만 원본 이관 + ERROR 로그로 바꿈

**폐쇄망 검증 완료 (2026-09-22)**
- [x] 폐쇄망 IBM J9의 `Cipher.getMaxAllowedKeyLength("AES")` 실측 — **256bit** (개발기와 동일, 128bit 폴백 없이 최상 케이스)
- [x] 키 파일 600 권한 + `db_config`와 다른 장소에 사본 1부 보관 완료
- [x] 폐쇄망 Oracle 인스턴스 실접속 회귀 확인 — 대시보드 실데이터 정상 표시(복호화된 비밀번호로 실접속 성공)
- [x] `dbagent-java.err.log` 비어있음, 정상종료→재기동으로 B64 옛값 잔재 정리 완료, 빈 비밀번호 저장 시 기존 값 유지 확인
- [x] 실서버 잔존 `oracle.env`/`databases.json` 삭제 완료(13절 잔여 항목)

6단계(비밀번호 AES 암호화) 폐쇄망 적용 **전 과정 완료.**
