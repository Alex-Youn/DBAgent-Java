...
- [ ] 스위치 "기존" 자리에 새 화면 연결 (라벨 변경, 마지막 선택값 저장은 기존 방식), 기존 게이지·탭 제거, v2 무변경
- [ ] 비Oracle DB면 새 화면·스위치 숨기고 기존 RDB 대시보드 그대로
- [ ] 상태바: 구간 버튼 15분/1시간/3시간/24시간(기본 1시간)
- [ ] 리프레쉬 입력창 기본 3초, 2~60 보정, 변경 즉시 적용
- [ ] 수동 새로고침 / 자동 갱신 중지·재개 연결
- [ ] 색 토큰: 기존 `style.css` 변수에 매핑, 라이트 테마(`data-theme="light"`) 값 정의
- [ ] 폐쇄망 제약: 외부 폰트·CDN 없음, `color-mix()`·`dvh` 없음 (rgba 토큰·vh)
- [ ] 첫 로딩 스켈레톤 (검은 빈 화면 없음)
- [ ] DB 전환: `AbortController` + DB 세대 번호로 늦은 응답 폐기, 드로어·패널 닫기
- [ ] 화면 숨김(`document.hidden`)·다른 탭 이동 시 폴링 중지, 복귀 시 1회 갱신 후 재개
- [ ] 권한 없음(403) 한 줄 표시
- [ ] 1920×1040 스크롤 없음 레이아웃 (flex, ②③ 행이 남는 높이), 작은 창은 대시보드 영역만 스크롤
- [ ] AIX 포팅
- [ ] 커밋
...
- [ ] 구간 선택: 최초·구간 변경 시 최대 부하 10분 자동 선택 / 클릭 5분 / 드래그 / 키보드 / "선택 해제"
- [ ] 선택 구간을 `dbClockOffsetMs`로 DB 시각 변환해 `/top` 요청
- [ ] 데이터 공백은 끊어 그림 + "수집 이전", 수집 지연 3분↑ 배지
- [ ] "표로 보기" (코어 초과 분 붉은 배경)
- [ ] ④ 진단 배너: 최대 비중 클래스 + 조치 문구(설계 5장 ④ 표), 코어 초과 문구, 빈 구간 문구
- [ ] zoom 90%에서 호버·드래그 좌표 확인
- [ ] AIX 포팅
- [ ] 커밋

---

## 8단계 — ③ Lock 카드 · ③-1 장애 처리 UI

- [ ] 상단 4칸: TX 대기, TM 대기, 블로킹 TM Holder(60초↑) `n / 6개`, 최장 last_call_et
- [ ] 계단형 차트, 최근 10분 **시간 기준** 버퍼, 경고선, 끝 라벨, 장애 구간 붉은 배경, KILL 세로선
- [ ] 헤더 `LIVE · n초 주기` / "일시정지됨"
- [ ] 판단 보류: 직전 값 흐리게, 0 안 찍음, 장애 표시 유지, KILL 버튼 비활성
- [ ] 장애 시 카드 붉게 + 알림 바 문구(설계 ③-1) + 버튼
- [ ] 비관리자: 버튼 대신 "장애 처리는 관리자만" 안내
- [ ] 확인 패널: 기본 전체 선택, `60초 미만` 표시, 구문 미리보기
- [ ] 확인 패널: 조건 해소 시 자동으로 닫지 않고 "장애 조건 해소됨" 안내
- [ ] 확인 패널: 종료 세션 `종료됨`+비활성, 새 Holder는 `새로 생김`+미선택
- [ ] 확인 패널: 취소/KILL/Esc로만 닫힘, DB 전환 시 닫힘, 요청 dbId는 패널 연 시점 값
- [ ] KILL 완료 후 처리 기록 1줄
- [ ] AIX 포팅
- [ ] 커밋
