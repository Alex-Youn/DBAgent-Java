...
- [x] 선택 구간을 `dbClockOffsetMs`로 DB 시각 변환해 `/top` 요청 — `selection` 이벤트(DB 시각) → F7 `nd-top.js`가 구간이 바뀔 때만 `/top` 호출
...
## 9단계 — ⑤⑥⑦ Top 목록 · 상세 드로어 ✅ 완료 (2026-09-26, 전체 순서 F7)

- [x] ⑤ Top SQL / ⑥ Top 세션 / ⑦ Top 이벤트 (5개, 구성 막대, 빈 결과 문구, ASH 조회 실패 문구)
- [x] 드로어: 오버레이, `body` 바로 아래 부착(zoom 버그 회피), Esc/배경/✕ 닫기, 포커스 복귀, 스택 뒤로가기
- [x] 세션 / SQL / 이벤트 상세, "조회 쿼리 보기"
- [x] 다른 인스턴스 블로커는 "다른 인스턴스(n번) SID m"으로만 표시
- [x] "KILL 구문 복사"(관리자만): `navigator.clipboard` → `execCommand('copy')` → 수동 복사 안내 순서
- [x] 접속 인스턴스명 + "이 인스턴스 기준" 표시
- [x] AIX 포팅
- [x] 커밋

---

## 10단계 — ⑧ 점검 알림 ✅ 완료 (2026-09-26, 전체 순서 F8)

- [x] 10분 점검 잡: 테이블스페이스(97/98), FRA(`NULLIF`, 미설정 제외), TEMP(`dba_temp_free_space`+`dba_temp_files` MAXSIZE), 잡 실패
- [x] 1일 점검 잡(`dbagent.monitor.check-daily-hour`, 기본 3시, 기동 직후 1회): 테이블 용량+세그먼트 스냅샷(`dba_segments` 1회), 무효 객체, 통계
- [x] 항목별 OK/ERROR 행 기록, 화면은 항목별 최신 결과 조회
- [x] 카드(위험→주의→확인, 점검 실패 회색), 필터, 항목별 마지막 점검 시각, 프레임 안 가로 스크롤
- [x] 상세 드로어(6.4): 7일 추이, 관련 객체, 조치 문구
- [x] 수집 계정 `dba_*` 권한 확인 (없으면 ERROR 카드)
- [x] AIX 포팅
- [x] `query-performance-reviewer` 검토 — 반영: 잡 실패 30분 주기 분리(날짜 인덱스 없는 풀스캔), 1일 점검 타임아웃을 read-timeout보다 5초 짧게 자동 제한, 관련 객체 60초 캐시, 점검 스레드 4개. 백로그: 관련 객체(테이블스페이스 top 세그먼트·테이블 인덱스)를 일 스냅샷으로 옮겨 클릭 시 원본 조회 없애기, 운영 규모에서 1일 점검 소요 시간 실측
- [x] 커밋

---

## 11단계 — 전체 검증 · 배포 ✅ 완료 (2026-09-26, 11g 실측만 폐쇄망 대기)

- [x] 설계 문서 10장 수용 기준 전체 확인 — `feature-tester` 대신 단계별(F4~F8) 실세션·CDP 검증 + 11단계에서 남은 기준만 추가 확인(다중 탭·라이트 테마·복사 대체·접속 실패·수집 지연). 11g만 미확인
- [x] 브라우저 실렌더 검증 (로컬 Chrome + CDP, 8099 테스트 포트): 1920×1040 스크롤 없음, zoom 80%(B1 이후 기준), 라이트 테마
- [x] 화면 상태 13종 각각 재현 확인 (설계 5장 "화면 상태 정의") — 첫 로딩·DB 전환·데이터 공백(AIX 실데이터)·수집 지연·접속 실패·판단 보류·ASH 조회 실패·빈 결과·권한 없음(403)·비관리자·비Oracle(기존 별도 페이지)·점검 실패·화면 숨김
- [ ] 11g 대상 DB 동작 확인 (가능하면) — 로컬 11g 없음, 폐쇄망 확인 요청(12c 전용 문법은 코드 검색으로 0건 확인, 기존 성능 이력 Top 세션의 FETCH FIRST는 발견해 ROWNUM으로 수정)
- [x] 여러 브라우저 탭으로 동시 조회 시 Lock 조회가 주기당 1회인지 확인 — 탭 3개·3초·31초 동안 v$sql 실행 11회(탭 1개 기준 ~10, 독립이면 ~31)
- [x] 메인 빌드·배포 (dist) — 캐시버스터 main app 182·style 59·nd-* / AIX app 170·style 52
- [x] AIX 빌드 (Java 8 타깃) 확인 — dist·dist-aix 배포, 스모크 통과
- [x] 폐쇄망 적용 절차서 작성 (새 프로퍼티 키 안내 포함) — `가이드/폐쇄망_적용절차_2026-09-26_배포5.md`
- [x] 작업정리 기록 + 메모리 갱신
- [x] 체크리스트 9번째-2 완료 표시
